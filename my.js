...
var playerNameElt = document.querySelector("#playerName");
...
// initiate the game with player name
playerNameElt.addEventListener("click", function (e) {
    if (e.target.classList.contains("playerName")) {
        activatePlayerName(e.target.id);
        playerNameElt.classList.add("hidden");
...
function activateTheme(theme) {
    ...
    for (let i = 0; i < 20; i++) {
        images.push(library4X4[theme][i]);
    }
...
function resetGame() {
    ...
    playerNameElt.classList.remove("hidden");
    for (let i = 0; i < 20; i++) {
